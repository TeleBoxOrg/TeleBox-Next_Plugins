...
class ZhijiaoPlugin extends Plugin {
  ...
  > = {
    zhijiao: async (msg: MessageContext, trigger?: MessageContext) => {
      ...
      await msg.edit({ text: html(buildProgressText(0)) });

      for (let i = 0; i < tosses.length; i++) {
        // mtcute sleep() takes milliseconds
        // reveal at ~800ms apiece so the animation actually plays.
        await sleep(800);
        await msg.edit({
          text: html(buildProgressText(i + 1)),
        });
      }
