...
/** 妙招四：curl_cffi 伪装 TLS 拉 DDG HTML */
async function fetchDdgViaCurlCffi(
  ...
  try {
    ...
    if (stderr && /Error|Traceback/i.test(stderr)) {
      logger.warn("[duckduckgo] ddg_fetch stderr", stderr.slice(0, 300));
    }
    ...
    logger.warn("[duckduckgo] curl_cffi DDG 失败", e);
    return { results: [], blocked: true, note: `ddg_fetch: ${msg.slice(0, 120)}` };
  }
}

/** 妙招二：Firecrawl keyless search */
async function fetchFirecrawl(
  ...
  try {
    ...
    // v2: { success, data: { web: [...] } }  v1: { success, data: [...] }
    ...
    logger.warn("[duckduckgo] Firecrawl 失败", e);
    return [];
  }
}
...
/** Telegram 文本上限 4096；HTML 标签开销预留，按「完整结果块」打包，绝不半路切断标签 */
const MSG_SOFT_LIMIT = 3500;
const MSG_HARD_LIMIT = 4000;

function buildHeader(query: string, total: number, sources: string[], elapsedMs: number): string {
  return (
...
/** 单条搜索结果 = 标题行 + 折叠 blockquote（原子块，不可拆开） */
...
function packSearchMessages(
  ...
  // 有序列表：每项完整、互不重叠
  type Chunk = { kind: "prelude" | "result" | "note" | "footer"; html: string; index?: number };
  const queue: Chunk[] = [];

  const preludeParts: string[] = [
    buildHeader(query, total, sources, elapsedMs),
    "",
  ];
  queue.push({ kind: "prelude", html: preludeParts.join("\n").replace(/\n+$/, "\n") });

  results.forEach((r, i) => {
    queue.push({ kind: "result", html: buildResultBlock(r, i + 1), index: i + 1 });
  });
  ...
  for (const chunk of queue) {
    ...
    if (chunk.kind === "result" && chunk.index) {
      startContinuation(chunk.index);
      page = canFit(page, chunk.html, MSG_HARD_LIMIT) ?? `${page}\n${chunk.html}`;
      pageLastIdx = chunk.index;
    } else {
      page = chunk.html;
    }
  ...
  if (pages.length > 1) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      } else if (pages[i].length + `\n\n📄 (${i + 1}/${n})`.length <= MSG_HARD_LIMIT) {
        pages[i] = pages[i] + `\n\n📄 (${i + 1}/${n})`;
      }
      // 若连短标记都塞不下，宁可不要标记，也不截正文
    }
  }
...
async function deliverSearchMessages(
  ...
  for (let i = 1; i < parts.length; i++) {
    ...
    try {
      ...
      logger.warn(`[duckduckgo] sendText 第 ${i + 1} 页失败，尝试 replyText`, e);
      await msg.replyText(html(part), { disableWebPreview: true });
    }
  }
}

async function handleSearch(msg: MessageContext): Promise<void> {
  ...
  if (query.length > 200) {
    await msg.edit({ text: html("❌ 关键词过长（最多 200 字符）") });
    return;
  }
  ...
  try {
    ...
    if (!runtime.ok) {
      ...
      await msg.edit({
        text: html(`⏳ 正在搜索 <code>${htmlEscape(query)}</code>…`),
      });
    }
    ...
    logger.error("[duckduckgo] 搜索失败", error);
    await msg.edit({
      text: html(
