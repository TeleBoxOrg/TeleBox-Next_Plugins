...
• 回复一条消息后发送 <code>${mainPrefix}yinglish</code>`;
  ...
  cmdHandlers: Record<string, (msg: MessageContext, trigger?: MessageContext) => Promise<void>> = {
    yinglish: async (msg: MessageContext) => {
      ...
      try {
        ...
        // 从回复消息获取文本
          ...
          try {
            const replyMsgId = msg.replyToMessage.id;
            const messages = await safeGetMessages(client, msg.chat.id, [replyMsgId]);
            
            if (messages && messages.length > 0 && messages[0].text) {
              text = messages[0].text;
            } else {
              await msg.edit({
                text: html`❌ 无法获取回复消息的内容`
              });
              return;
            }
          } catch (error) {
