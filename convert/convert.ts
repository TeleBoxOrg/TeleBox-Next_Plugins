...
class ConvertPlugin extends Plugin {
  ...
  cmdHandlers: Record<string, (msg: MessageContext) => Promise<void>> = {
    convert: async (msg: MessageContext) => {
        const parts = msg.text?.trim()?.split(/\s+/) || [];
        const args = parts.slice(1);
        const subCommand = (args[0] || "").toLowerCase();

        try {
            if (subCommand === "help" || subCommand === "h" || args.length === 0 && !msg.replyToMessage) {
  ...
  private async handleVideoConversion(msg: MessageContext, args: string[]): Promise<void> {
    ...
    try {
        ...
        if (useAi && userQuery) {
            ...
            await msg.edit({ text: "🤖 AI 正在识别歌曲信息..." });
  ...
  private async handleClearCommand(msg: MessageContext): Promise<void> {
    await msg.edit({ text: "🧹 正在清理临时文件..." });
    converter.cleanupAllTempFiles();
    await msg.edit({ text: "✅ 临时文件已清理" });
  }

  private async handleApiKeyCommand(msg: MessageContext, apiKey: string): Promise<void> {
    if (!apiKey) {
