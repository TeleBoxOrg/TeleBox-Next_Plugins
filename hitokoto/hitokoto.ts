...
class HitokotoPlugin extends Plugin {
   ...
   */
  private async handleHitokotoCommand(msg: MessageContext): Promise<void> {
    try {
   ...
   * 获取并发送一言
   */
  private async fetchAndSendHitokoto(msg: MessageContext, queryParams?: Record<string, string | string[]>): Promise<void> {
    // 发送等待消息
    await msg.edit({
      text: "🔄 正在获取一言..."
    });
