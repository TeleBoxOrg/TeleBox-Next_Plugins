...
cmdHandlers:Record<string,(msg:MessageContext)=>Promise<void>>={checkapi:async(msg)=>{
  const raw=msg.text.slice(mp.length).trim();
  ...
  // ── list / del / save / check ──
  if(sub==="list"){const keys=await lk();if(!keys.length){await msg.edit({text:html`📭 无保存\n<code>${mp}checkapi save &lt;name&gt; &lt;key&gt;</code>`});return;}const lines=[`密钥 (${keys.length}):`];for(const k of keys)lines.push(`  • <b>${k.name}</b>: ${mk(k.key)} (${k.provider||"auto"})${k.baseUrl?` [${(()=>{try{return new URL(k.baseUrl).hostname}catch{return k.baseUrl}})()}]`:""}`);await msg.edit({text:html`${lines.join("\n")}`});return;}
  if(sub==="del"||sub==="delete"){const name=parts[1];if(!name){await msg.edit({text:html`❌ <code>${mp}checkapi del &lt;名称&gt;</code>`});return;}const keys=await lk();const idx=keys.findIndex(k=>k.name===name);if(idx===-1){await msg.edit({text:html`❌ <b>${name}</b> 不存在`});return;}keys.splice(idx,1);await sk(keys);await msg.edit({text:html`✅ 已删 <b>${name}</b>`});return;}
  if(sub==="save"){let name:string|undefined,key:string|undefined,baseUrl:string|undefined;if(parts.length>=4&&isUrl(parts[2])){name=parts[1];baseUrl=nu(parts[2]);key=parts[3];}else{name=parts[1];key=parts[2];baseUrl=parts[3];}if(!name||!key){await msg.edit({text:html`❌ <code>${mp}checkapi save &lt;名称&gt; &lt;key&gt; [url]</code>`});return;}const keys=await lk();const info=dp(key,baseUrl);const entry:SK={name,key,baseUrl,provider:info.provider,addedAt:Date.now()};const idx=keys.findIndex(k=>k.name===name);if(idx>=0)keys[idx]=entry;else keys.push(entry);await sk(keys);await msg.edit({text:html`✅ ${idx>=0?"已更新":"已保存"} <b>${name}</b> → <code>${mp}checkapi ${name}</code>`});return;}
  if(sub==="check"){const target=parts[1]||"all";if(target==="all"){const keys=await lk();if(!keys.length){await msg.edit({text:html`📭 无保存`});return;}await msg.edit({text:html`检测 ${keys.length} 个...`});const results:string[]=[];const promises=keys.map(async(k)=>{const info=dp(k.key,k.baseUrl);try{return[`\n━━━ <b>${k.name}</b> ━━━`,...(await fcv2(info.provider,k.key,k.baseUrl||info.baseUrl))]as string[]}catch(e:unknown){return[`\n━━━ <b>${k.name}</b> ━━━`,`⚠️ ${ge(e)}`]as string[]}});const batches=await Promise.all(promises);for(const row of batches)results.push(...row);await msg.edit({text:html`${results.join("\n")}`});return;}const keys=await lk();const found=keys.find(k=>k.name===target);if(!found){await msg.edit({text:html`❌ <b>${target}</b> 不存在`});return;}await msg.edit({text:html`🔍 <b>${target}</b>...`});const info=dp(found.key,found.baseUrl);const results=await fcv2(info.provider,found.key,found.baseUrl||info.baseUrl);await msg.edit({text:html`${results.join("\n")}`});return;}

  // ── models / ask / speed ──
  if(sub==="models"){const input=parts[1];if(!input){await msg.edit({text:html`❌ <code>${mp}checkapi models &lt;key|name&gt;</code>`});return;}const keys=await lk();const found=keys.find(k=>k.name===input);const key=found?found.key:input;const info=found?.baseUrl?await probeApi(found.baseUrl,key):dp(key);await msg.edit({text:html`🔍 ${info.displayName} 模型列表...`});const result=await lmf(info.provider,key,info.baseUrl);await msg.edit({text:html`${result}`});return;}
  if(sub==="ask"){const input=parts[1];const question=parts.slice(2).join(" ");if(!input){await msg.edit({text:html`❌ <code>${mp}checkapi ask &lt;key|name&gt; &lt;问题&gt;</code>`});return;}const keys=await lk();const found=keys.find(k=>k.name===input);const key=found?found.key:input;const info=dp(key,found?.baseUrl);const prompt=question||"say hello";await msg.edit({text:html`💬 ${info.displayName}: "${prompt}" ...`});const chat=await ct(info.provider,key,info.baseUrl,prompt);if(chat.ok){const l=[`💬 <b>${info.displayName}</b>`,`🤖 <code>${chat.model}</code> | 🕐 ${chat.elapsedMs}ms`,`📝 ${chat.text}`];if(chat.usage)l.push(`📊 入${chat.usage.prompt} 出${chat.usage.completion} 计${chat.usage.total}`);if(chat.headers)l.push(fh(chat.headers));await msg.edit({text:html`${l.join("\n")}`});}else{await msg.edit({text:html`❌ (${chat.elapsedMs||"?"}ms): ${chat.error}`});}return;}
  if(sub==="speed"){const input=parts[1];if(!input){await msg.edit({text:html`❌ <code>${mp}checkapi speed &lt;key|name&gt;</code>`});return;}const keys=await lk();const found=keys.find(k=>k.name===input);const key=found?found.key:input;const info=dp(key,found?.baseUrl);await msg.edit({text:html`⚡ ${info.displayName} 基准测试中...`});const results=await speedTest(info.provider,key,info.baseUrl);await msg.edit({text:html`${results.join("\n")}`});return;}

  // ── compare: two keys side by side ──
  if(sub==="compare"){const [a,b]=[parts[1],parts[2]];if(!a||!b){await msg.edit({text:html`❌ <code>${mp}checkapi compare &lt;key1|name1&gt; &lt;key2|name2&gt;</code>`});return;};const keys=await lk();const resolve=(input:string)=>{const f=keys.find(k=>k.name===input);return f?{key:f.key,baseUrl:f.baseUrl,label:f.name}:{key:input,baseUrl:undefined,label:mk(input)};};const r1=resolve(a),r2=resolve(b);const p1=dp(r1.key,r1.baseUrl),p2=dp(r2.key,r2.baseUrl);await msg.edit({text:html`🔍 对比 <b>${r1.label}</b> vs <b>${r2.label}</b>...`});const [s1,s2]=await Promise.all([fcv2(p1.provider,r1.key,p1.baseUrl),fcv2(p2.provider,r2.key,p2.baseUrl)]);const m=[`⚖️ <b>${p1.displayName}</b> (${r1.label})`,...s1,`\n━━━━━━━━━━━━━━━━`,...s2];await msg.edit({text:html`${m.join("\n")}`});return;}
