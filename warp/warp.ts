...
class WarpPlugin extends Plugin {
  ...
  // 子命令帮助
  private async showSubCommandHelp(subCmd: string, msg: MessageContext): Promise<void> {
  ...
  // 主处理
  private async handleWarp(msg: MessageContext): Promise<void> {
    // 标准参数解析模式（参考规范）
    ...
    try {
      ...
      switch (sub) {
        ...
        case "status": {
          await msg.edit({ text: html("🔄 正在获取状态...") });
          const statusText = await WireproxyManager.getStatus();
          await msg.edit({ text: html(statusText) });
          return;
        }
        ...
        case "w": {
          ...
          await msg.edit({ text: html(ret) });
          return;
        }

        case "stop": {
          await msg.edit({ text: html("🔄 正在停止 wireproxy...") });
        ...
        case "ip": {
          await msg.edit({ text: html("🔄 正在重启 wireproxy (更换IP)...") });
        ...
        case "uninstall":
        case "uninstall_all": {
          await msg.edit({ text: html("⚠️ 正在卸载 wireproxy...") });
        ...
        case "y": {
          await msg.edit({ text: html("🔄 正在切换 WireProxy 状态...") });
        ...
        case "e": {
          ...
          await msg.edit({ text: html("🔄 正在安装 Iptables + dnsmasq + ipset 方案...") });
          try {
        ...
        case "proxy": {
          await msg.edit({ text: html("🔄 正在配置代理设置...") });
        ...
        case "unproxy": {
          await msg.edit({ text: html("🔄 正在关闭代理设置...") });
        ...
        case "music": {
          await msg.edit({ text: html("🔄 正在配置 Music 插件代理...") });
        ...
        case "unmusic": {
          await msg.edit({ text: html("🔄 正在关闭 Music 插件代理...") });
